    ...
    float* dV) -> void
{
    ...
    uint16_t k = (step <= nWave) ? step : nWave;
    ...
    const bool& walls) -> void
{
    ...
    uint16_t nWave;
    {
        const float part_mass{1.f};
        // The maximum waves to store in the array
        const uint16_t nWave_max = static_cast<uint16_t>((3.f*wave_range)/(wave_speed*dT) + 1.f);
        // The effective waves amount to store in the waves array
        nWave = (nWave_max < nStep) ? nWave_max : nStep;
    
        #pragma omp parallel for default(none) \
            shared(particles_array, lBox, wave_speed, wave_width, wave_range, \
                    nDim, nWave, nParticles, part_mass)
        for(uint16_t p = 0; p < nParticles; p++)
        {
            particles_array[p] = particle(lBox, part_mass, &wave_speed, &wave_width, &wave_range,
                        &nDim, &nWave, p);
        }
    }

    {
        ...
        float dV[nDim];
        const uint8_t nNeighbors = walls ? 0u : static_cast<uint8_t>((6.f*wave_range)/(lBox) + 1u);
        uint16_t i;
